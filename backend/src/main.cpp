...
int main() {
    ...
    bus->subscribe("market.ticks", [&](const void* data, size_t size) {
        argentum::bus::MessageHeader header{};
        if (!argentum::bus::decode_header(data, size, &header)) return;
        if (header.type != static_cast<uint16_t>(argentum::bus::MessageType::MarketTick)) return;
        if (size < sizeof(argentum::bus::MessageHeader) + header.size) return;
        if (header.size != sizeof(MarketTick)) return;

        const uint8_t* payload = argentum::bus::payload_ptr(data, size);
        if (!payload) return;
        MarketTick tick{};
        std::memcpy(&tick, payload, sizeof(MarketTick));
        writer.enqueue(tick);
    });
    ...
    if (published == 0) {
        ...
        tick.side = SIDE_BUY;
        auto payload = argentum::bus::encode_message(argentum::bus::MessageType::MarketTick, &tick, sizeof(tick), tick.timestamp_ns);
        bus->publish("market.ticks", payload.data(), payload.size());
        published = 1;
    }
