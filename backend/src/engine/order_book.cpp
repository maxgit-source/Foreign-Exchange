...
bool OrderBook::add_order(const Order& order) {
    ...
    auto& book_side = (order.side == SIDE_BUY) ? bids_ : asks_;
...
bool OrderBook::cancel_order(uint64_t order_id) {
    ...
    auto& book_side = (loc.side == SIDE_BUY) ? bids_ : asks_;
...
std::vector<Trade> OrderBook::match_order(const Order& incoming) {
    ...
    auto& book_side = (incoming.side == SIDE_BUY) ? asks_ : bids_;

    auto it = book_side.begin();
    while (it != book_side.end() && remaining > 0.0) {
        double level_price = it->first;

        if (incoming.type == ORDER_TYPE_LIMIT) {
            if (incoming.side == SIDE_BUY && level_price > incoming.price) break;
            if (incoming.side == SIDE_SELL && level_price < incoming.price) break;
        }
...
std::optional<double> OrderBook::vwap(Side side, double quantity) const {
    ...
    const auto& book_side = (side == SIDE_BUY) ? asks_ : bids_;
    double remaining = quantity;
    double notional = 0.0;

    for (const auto& [price, orders] : book_side) {
